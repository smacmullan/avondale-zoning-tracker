...
# Filepaths
GEOCODE_CSV = "data/out/geocode_results.csv"
ORDINANCES_CSV = "data/out/ordinance_export_with_address.csv"
...
OUTPUT_CSV = "data/out/avondale_zoning.csv"
OUTPUT_CITYWIDE_CSV = "data/out/citywide_zoning.csv"

# Connect and enable spatial extension
con = duckdb.connect()
con.execute("INSTALL spatial;")
con.execute("LOAD spatial;")
print("DuckDB started.")

# Load geocode CSV with pandas and pass to DuckDB (DuckDB's read_csv_auto had issues with format)
df_geo = pd.read_csv(GEOCODE_CSV, dtype=str, quotechar='"', skip_blank_lines=True)
con.register("geocode_raw", df_geo)

# Extract coordinates in a subquery
con.execute("""
...
# Read in ordinances CSV
con.execute(f"""
CREATE OR REPLACE TABLE ordinances AS
SELECT * FROM read_csv_auto('{ORDINANCES_CSV}', header=True);
""")
...
""")
print("Data loaded. Starting spatial analysis...")
...
con.execute(
    f"COPY (SELECT * EXCLUDE geom from zoning_requests) TO '{OUTPUT_CITYWIDE_CSV}' (HEADER TRUE);"
)
print(f"Wrote {OUTPUT_CITYWIDE_CSV}.")
...
# filter on points on avondale buffer
# remove points that are across the river (North Center)
con.execute("""
...
con.execute(
    f"COPY (SELECT * EXCLUDE geom FROM points_within_avondale) TO '{OUTPUT_CSV}' (HEADER TRUE);"
)

row = con.execute("SELECT COUNT(*) FROM points_within_avondale").fetchone()
count = row[0] if row is not None else 0
print(f"Wrote {OUTPUT_CSV}. Query returned {count} rows.")
